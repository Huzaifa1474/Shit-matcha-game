...
type CampaignProgress = { credits: number; completed: MissionKey[]; medals: MissionKey[]; achievements: string[]; battlesWon: number; campaignComplete: boolean; ownedFrames: LoadoutKey[] };
...
const INITIAL_HUD: HudState = {
  ...
  juggleActive: false,
  juggleHeight: 0,
  followUpHint: "LAUNCH // F+J RAIL LIFTER",
  overdrive: false,
  specialCooldown: 0,
  specialReady: true,
...
const DEFAULT_PROGRESS: CampaignProgress = { credits: 480, completed: [], medals: [], achievements: [], battlesWon: 0, campaignComplete: false, ownedFrames: ["vanguard"] };
...
const BOSS_INTROS: Record<string, BossIntroData> = Object.fromEntries(CAMPAIGN_LEVELS.filter((mission) => Boolean(mission.boss)).map((mission) => [mission.key, { missionKey: mission.key, threat: `${mission.stageLabel} // COMMANDER`, title: mission.boss!.title, designation: mission.boss!.designation, directive: mission.boss!.directive, transmission: mission.boss!.transmission, artUrl: mission.boss!.artUrl, accent: mission.boss!.accent }])) as Record<string, BossIntroData>;
...
function FrameSpecimen({ profile, index }: { profile: MechaProfile; index: number }) {
  const artUrl = AUTHORED_FRAME_ART[profile.key];
  if (artUrl) return <span className={`frame-specimen frame-art chassis-${profile.chassis} specimen-${index % 5}`} data-has-art="true" style={{ color: profile.accent }}><i className="specimen-halo" /><img src={artUrl} alt={`${profile.label} combat frame`} /></span>;
  return <span className={`frame-specimen chassis-${profile.chassis} specimen-${index % 5}`} data-has-art="false" style={{ color: profile.accent }} aria-label={`${profile.label} technical frame record; authored portrait pending`}><i className="specimen-halo" /><i className="specimen-engine engine-a" /><i className="specimen-engine engine-b" /><i className="specimen-helmet" /><i className="specimen-torso" /><i className="specimen-shoulder shoulder-a" /><i className="specimen-shoulder shoulder-b" /><i className="specimen-arm arm-a" /><i className="specimen-arm arm-b" /><i className="specimen-weapon" /><i className="specimen-leg leg-a" /><i className="specimen-leg leg-b" /><b className="specimen-core" /></span>;
}

function CompareMetric({ label, left, right, suffix = "", lowerBetter = false, precision = 0 }: { label: string; left: number; right: number; suffix?: string; lowerBetter?: boolean; precision?: number }) {
  const leftLead = left !== right && (lowerBetter ? left < right : left > right);
  const rightLead = left !== right && (lowerBetter ? right < left : right > left);
  const format = (value: number) => `${value.toFixed(precision)}${suffix}`;
  return <div className="compare-metric"><b className={leftLead ? "lead" : ""}>{format(left)}</b><span>{label}</span><b className={rightLead ? "lead" : ""}>{format(right)}</b></div>;
}

function ComparisonFrame({ profile, slot, owned, active, index, onAssign }: { profile: MechaProfile; slot: "A" | "B"; owned: boolean; active: boolean; index: number; onAssign: () => void }) {
  const tree = COMBO_TREES[profile.chassis];
  return <article className={`comparison-frame chassis-${profile.chassis} ${active ? "active" : ""}`} style={{ "--frame-accent": profile.accent } as React.CSSProperties}>
    <header><span>SLOT {slot} // {profile.callsign}</span><b>{owned ? "OWNED" : profile.unlock?.kind === "reward" ? `LINK ${String(profile.unlock.level).padStart(2, "0")}` : `${profile.unlock?.cost} SALVAGE`}</b></header>
    <div className="comparison-specimen"><FrameSpecimen profile={profile} index={index} /></div>
    <div className="comparison-copy"><strong>{profile.label}</strong><small>{profile.chassis.toUpperCase()} CHASSIS // {profile.collection}</small><p>{profile.description}</p><dl><dt>SPECIAL</dt><dd>Q // {profile.special?.name}</dd><dt>LAUNCH</dt><dd>{tree.launcher.command} // {tree.launcher.name}</dd><dt>FINISH</dt><dd>{tree.finisher.command} // {tree.finisher.name}</dd></dl></div>
    <footer>{owned ? <button type="button" onClick={onAssign}>{active ? "ACTIVE FRAME" : "ASSIGN FRAME"}</button> : <span>RECOVERY REQUIRED</span>}</footer>
  </article>;
}
...
function CombatStage({ hud }: { hud: HudState }) {
  ...
  return <section className={`css-battle-stage theatre-${hud.theatreClass} level-stage-${hud.stage} surface-${hud.surface} atmosphere-${hud.atmosphere}`} aria-hidden="true">
    ...
    <CustomMecha side="player" action={hud.playerState} position={hud.playerX} artUrl={AUTHORED_FRAME_ART[hud.selectedLoadout] ?? AEGIS_ART} chassis={playerChassis} surface={hud.surface} specialName={playerSpecial?.name} specialKind={playerSpecial?.kind} />
...
function initialBossIntro() {
  ...
  const mission = requested ? MISSIONS[requested] : undefined;
  const boss = mission?.boss;
  return boss && mission ? { missionKey: mission.key, threat: `${mission.stageLabel} // COMMANDER`, title: boss.title, designation: boss.designation, directive: boss.directive, transmission: boss.transmission, artUrl: boss.artUrl, accent: boss.accent } : null;
}
...
export default function GameCanvas() {
  ...
  const [comparisonLeft, setComparisonLeft] = useState<LoadoutKey>("vanguard");
  const [comparisonRight, setComparisonRight] = useState<LoadoutKey>("ironclad");
  ...
  useEffect(() => {
    ...
    setProgress((current) => {
      ...
      const ownedFrames = new Set(current.ownedFrames ?? ["vanguard"]);
      if (fresh) Object.values(LOADOUTS).forEach((profile) => { if (profile.unlock?.kind === "reward" && profile.unlock.level && profile.unlock.level <= hud.missionLevel) ownedFrames.add(profile.key); });
      return { credits, completed, medals: Array.from(medals), battlesWon: current.battlesWon + 1, achievements: Array.from(achievements), campaignComplete: current.campaignComplete || hud.missionKey === "level-20", ownedFrames: Array.from(ownedFrames) };
    });
  ...
  const activeTree = COMBO_TREES[selected.chassis];
  ...
  const stageProgress = ([1, 2, 3] as CampaignStage[]).map((stage) => ({ stage, secured: CAMPAIGN_LEVELS.filter((item) => item.stage === stage && completeSet.has(item.key)).length, total: CAMPAIGN_LEVELS.filter((item) => item.stage === stage).length }));
  const allFrames = Object.values(LOADOUTS);
  const ownedSet = new Set(progress.ownedFrames ?? ["vanguard"]);
  const ownedFrames = allFrames.filter((profile) => ownedSet.has(profile.key));
  ...
  const buyFrame = (profile: MechaProfile) => {
    const cost = profile.unlock?.cost;
    if (!cost || ownedSet.has(profile.key) || progress.credits < cost) return;
    setProgress((current) => ({ ...current, credits: current.credits - cost, ownedFrames: Array.from(new Set([...(current.ownedFrames ?? ["vanguard"]), profile.key])) }));
    setLoadout(profile.key);
  };
  const comparisonLeftProfile = LOADOUTS[comparisonLeft] ?? selected;
  const comparisonRightProfile = LOADOUTS[comparisonRight] ?? selected;
  const comparisonBay = <section className="comparison-bay" aria-label="Frame comparison bay">
    <header className="comparison-heading"><div><span>FRAME COMPARISON // TWO-SLOT DIAGNOSTIC</span><h3>COMPARE LOADOUTS</h3><p>Pin any two recovered or archived frames to evaluate their combat geometry before a deployment.</p></div><div className="comparison-actions"><label>SLOT A<select value={comparisonLeft} onChange={(event) => setComparisonLeft(event.target.value)}>{allFrames.map((profile) => <option value={profile.key} key={profile.key}>{profile.label}</option>)}</select></label><button type="button" onClick={() => { const left = comparisonLeft; setComparisonLeft(comparisonRight); setComparisonRight(left); }}>⇄ SWAP</button><label>SLOT B<select value={comparisonRight} onChange={(event) => setComparisonRight(event.target.value)}>{allFrames.map((profile) => <option value={profile.key} key={profile.key}>{profile.label}</option>)}</select></label></div></header>
    <div className="comparison-stage">
      <ComparisonFrame profile={comparisonLeftProfile} slot="A" owned={ownedSet.has(comparisonLeftProfile.key)} active={loadout === comparisonLeftProfile.key} index={allFrames.indexOf(comparisonLeftProfile)} onAssign={() => { setLoadout(comparisonLeftProfile.key); setView("hangar"); }} />
      <div className="comparison-metrics"><span>COMBAT DELTA</span><CompareMetric label="INTEGRITY" left={comparisonLeftProfile.maxHp} right={comparisonRightProfile.maxHp} /><CompareMetric label="MOBILITY" left={comparisonLeftProfile.speed} right={comparisonRightProfile.speed} precision={2} /><CompareMetric label="STRIKE" left={comparisonLeftProfile.strikeDamage} right={comparisonRightProfile.strikeDamage} /><CompareMetric label="GUARD RED." left={Math.round((1 - comparisonLeftProfile.guardMultiplier) * 100)} right={Math.round((1 - comparisonRightProfile.guardMultiplier) * 100)} suffix="%" /><CompareMetric label="SPECIAL DMG" left={comparisonLeftProfile.special?.damageMultiplier ?? 0} right={comparisonRightProfile.special?.damageMultiplier ?? 0} suffix="×" precision={2} /><CompareMetric label="SPECIAL RANGE" left={comparisonLeftProfile.special?.range ?? 0} right={comparisonRightProfile.special?.range ?? 0} precision={2} /><CompareMetric label="COOLDOWN" left={comparisonLeftProfile.special?.cooldown ?? 0} right={comparisonRightProfile.special?.cooldown ?? 0} suffix="s" lowerBetter precision={2} /></div>
      <ComparisonFrame profile={comparisonRightProfile} slot="B" owned={ownedSet.has(comparisonRightProfile.key)} active={loadout === comparisonRightProfile.key} index={allFrames.indexOf(comparisonRightProfile)} onAssign={() => { setLoadout(comparisonRightProfile.key); setView("hangar"); }} />
    </div>
  </section>;
  ...
  return <main className={`mecha-app ${settings.reducedFx ? "fx-reduced" : ""} ${settings.scanlines ? "scanlines-on" : "scanlines-off"} ${settings.contrast ? "contrast-on" : ""}`}>
    ...
    {view === "battle" ? <>
      ...
      <section className="battle-hud" aria-label="Battle interface">
        ...
        <div className="round-dock" aria-live="polite"><div className="round-readout">LVL-{String(hud.missionLevel).padStart(2, "0")} // R0{hud.round}<br />BEST OF 03</div><div className="location-readout"><span>{hud.locationLabel}</span><small>{hud.locationCallout}</small></div><div className={`route-readout ${hud.routeFinisherArmed ? "armed" : ""}`}><b>{hud.routeFinisherArmed ? activeRoute.finisher : activeRoute.label}</b><span>{hud.routeFinisherArmed ? "FINISHER ARMED" : `ROUTE ${hud.routeStep}/${hud.routeLength} · NEXT ${hud.routeNextInput.toUpperCase()}`}</span></div><div className={`juggle-readout ${hud.juggleActive ? "active" : ""}`}><b>{hud.juggleActive ? `AIR ${Math.round(hud.juggleHeight * 100)}%` : hud.activeMoveName || activeTree.label}</b><span>{hud.juggleActive ? hud.followUpHint : activeTree.string}</span></div>{hud.combo > 0 && <div className={`combo-readout ${hud.overdrive ? "overdrive" : ""}`}><b>{hud.combo}X</b><span>{hud.comboDamage} DMG · {hud.overdrive ? "OVERDRIVE" : "COMBO"}</span></div>}{hud.counterStatus && <div className="commander-counter">{hud.counterStatus}</div>}<div className="status-copy">{hud.message}</div></div>
        <footer className="bottom-deck"><aside className="plate manual"><div className="manual-header"><span className="info-label">{hud.stageLabel}</span><span className="system-line">{hud.playerState.toUpperCase()}</span></div><p>{hud.missionObjective}</p><dl><dt>MOVE</dt><dd>A / D or ← / →</dd><dt>MELEE</dt><dd>J or SPACE</dd><dt>LAUNCH</dt><dd>{activeTree.launcher.command} // {activeTree.launcher.name}</dd><dt>JUGGLE</dt><dd>{activeTree.followUp.command} → {activeTree.finisher.command}</dd><dt>PRISM GUARD</dt><dd>HOLD K</dd><dt>VECTOR BOOST</dt><dd>L or SHIFT</dd><dt>FRAME SPECIAL</dt><dd>Q // {activeSpecial?.name ?? "SPECIAL"}</dd><dt>RETREAT</dt><dd>{activeTree.backstep.command} // {activeTree.backstep.name}</dd><dt>OVERDRIVE</dt><dd>LAND 3 CLEAN HITS</dd></dl></aside><div className="action-cluster"><ActionButton action="strike" className="strike" onAction={onAction}><strong>J</strong>MELEE</ActionButton><ActionButton action="guard" className="guard" onAction={onAction}><strong>K</strong>PRISM GUARD</ActionButton><ActionButton action="boost" className="boost" onAction={onAction}><strong>L</strong>BOOST</ActionButton><ActionButton action="special" className={`special ${hud.specialReady ? "ready" : "cooling"}`} onAction={onAction}><strong>Q</strong><span className="special-button-name">{activeSpecial?.name ?? "SPECIAL"}</span><span className="special-cooldown"><i style={{ width: `${cooldownRatio * 100}%` }} /><small>{hud.specialReady ? "READY" : `${hud.specialCooldown.toFixed(1)}S`}</small></span></ActionButton></div><div className="direction-pad"><ActionButton action="left" onAction={onAction}><span>◀</span></ActionButton><ActionButton action="right" onAction={onAction}><span>▶</span></ActionButton></div></footer>
      </section>
      ...
      <div className="console-content">
        {view === "command" && <section className="command-bay"><header className="command-bay-heading"><span>COMMAND BAY // OPERATOR UPLINK</span><b>SELECT A LINK. ARM A FRAME. BREAK THE HOSTILE LINE.</b><small>RUSTBELT COMBAT NETWORK // {completeLevels.length}/20 LINKS SECURED</small></header><div className="command-bay-grid"><article className="deployment-module"><div className="module-topline"><span>NEXT COMBAT LINK</span><em>LVL {String(mission.level).padStart(2, "0")}</em></div><h1>{mission.title}<i>{mission.location.label}</i></h1><p>{mission.objective}</p><div className="module-specs"><span><b>DECK</b>{mission.location.callout}</span><span><b>HOSTILE</b>{mission.opponent.label}</span><span><b>PRESSURE</b>AI {mission.difficulty}/20</span></div><button type="button" className="primary-console-button" onClick={() => setView("missions")}>OPEN LEVEL BOARD</button></article><article className="frame-gantry"><div className="gantry-rail gantry-left" /><div className="gantry-rail gantry-right" /><div className="gantry-bay-label">FRAME SERVICE LIFT // {selected.callsign}</div><div className="frame-core"><span className="core-signal" /><img src={AUTHORED_FRAME_ART[selected.key] ?? AEGIS_ART} alt={`${selected.label} active combat frame`} /></div><div className="frame-readout"><span>ACTIVE FRAME</span><b>{selected.label}</b><small>Q // {selected.special?.name}</small><em>{activeTree.string} // {activeTree.finisher.name}</em></div><button type="button" className="frame-service-button" onClick={() => setView("hangar")}>REFIT FRAME</button></article><aside className="campaign-rail-console"><div className="rail-title"><span>CAMPAIGN RAIL</span><b>THREE THEATRES</b></div><div className="rail-path">{stageProgress.map((item) => <button type="button" className={`rail-stop ${item.secured === item.total ? "cleared" : item.stage === Math.min(3, Math.ceil(unlockedLevel / 7)) ? "active" : ""}`} onClick={() => { setStageFilter(item.stage); setView("missions"); }} key={item.stage}><i>0{item.stage}</i><span>STAGE {item.stage}</span><small>{item.secured}/{item.total} SECURED</small></button>)}</div><div className="rail-footer"><span>ACTIVE SURFACE</span><b>{mission.location.surface.toUpperCase()}</b><small>REACTOR OUTPUT // NOMINAL</small></div></aside></div><div className="command-bay-links"><button type="button" onClick={() => setView("missions")}><i>⌖</i><span>COMBAT LINKS</span><small>{unlockedLevel}/20 UNLOCKED</small></button><button type="button" onClick={() => setView("roster")}><i>▥</i><span>FRAME GARAGE</span><small>{selected.label}</small></button><button type="button" onClick={() => setView("achievements")}><i>✦</i><span>WAR RECORD</span><small>{progress.achievements.length}/4 BADGES</small></button></div></section>}
        {view === "missions" && <section className="console-panel campaign-board"><div className="panel-heading"><span>CAMPAIGN BOARD // 20 LEVELS</span><h2>SELECT A COMBAT LINK</h2><p>Each new level changes the fight location, surface response, arena atmosphere, hostile AI pressure, and salvage yield.</p></div><div className="stage-tabs">{([1, 2, 3] as CampaignStage[]).map((stage) => <button type="button" className={stageFilter === stage ? "active" : ""} onClick={() => setStageFilter(stage)} key={stage}>STAGE {stage}</button>)}</div><div className={`stage-rail stage-${stageFilter}`}><div className="stage-rail-header"><span>{stageLevels[0].stageLabel}</span><b>{stageLevels[0].theatre}</b></div><div className="level-grid">{stageLevels.map((item) => { const complete = completeSet.has(item.key); const locked = item.level > unlockedLevel; return <button type="button" disabled={locked} className={`level-card ${complete ? "complete" : ""} ${locked ? "locked" : ""} ${item.boss ? "commander-link" : ""}`} onClick={() => selectMission(item.key)} key={item.key}><span>LEVEL {String(item.level).padStart(2, "0")}</span>{item.boss && <em className="boss-stamp">COMMANDER // {item.boss.title}</em>}<strong>{item.title}</strong><i>{item.location.label}</i><small>{locked ? "LINK LOCKED" : complete ? "SECURED" : item.boss ? item.boss.behavior : item.location.callout}</small><footer><em>AI {item.difficulty}/20</em><b>+{item.reward}</b></footer></button>; })}</div></div></section>}
        {view === "hangar" && <section className="console-panel hangar-console"><div className="panel-heading"><span>{mission.stageLabel} // LEVEL {String(mission.level).padStart(2, "0")}</span><h2>ASSIGN AN OWNED FRAME</h2><p>{mission.objective} Hostile: <strong>{mission.boss ? mission.boss.title : mission.opponent.label}</strong>.</p></div><div className="hangar-showcase"><div className="hangar-specimen"><FrameSpecimen profile={selected} index={allFrames.indexOf(selected)} /></div><div><b>ACTIVE PILOT FRAME</b><strong>{selected.label}</strong><p>{selected.description}</p><span>FRAME SPECIAL // {selected.special?.name} · {selected.special?.tactical}</span><span>COMBO ROUTE // {activeRoute.label} · {activeRoute.sequence.map((step) => step === "special" ? "Q" : "J").join(" → ")} → {activeRoute.finisher}</span></div></div><div className="owned-frame-strip">{ownedFrames.map((profile, index) => { const route = COMBO_ROUTES[profile.key]; return <button type="button" className={`owned-frame-card ${profile.chassis} ${loadout === profile.key ? "selected" : ""}`} onClick={() => setLoadout(profile.key)} key={profile.key}><FrameSpecimen profile={profile} index={index} /><span className="loadout-code">{profile.callsign}</span><strong>{profile.label}</strong><small>Q // {profile.special?.name}</small><em>{route.finisher}</em></button>; })}</div><div className="hangar-launch-row"><button type="button" className="secondary-console-button" onClick={() => setView("roster")}>OPEN FRAME COLLECTION</button><button type="button" className="primary-console-button" onClick={launch}>LAUNCH LEVEL {String(mission.level).padStart(2, "0")}</button></div></section>}
        {view === "roster" && <section className="console-panel collection-console">
          <div className="collection-heading"><div><span>BLUE FACTION // FRAME COLLECTION</span><h2>ARMORY // {ownedFrames.length}/20 ACTIVE</h2><p>Maintenance-bay records for every recovered chassis. Secure links or spend salvage to bring new frames onto the rail.</p></div><aside><b>{progress.credits.toLocaleString()}</b><span>SALVAGE AVAILABLE</span><small>REWARD FRAMES UNLOCK ON THEIR LINK CLEAR</small></aside></div>
          {comparisonBay}
          <div className="collection-grid">{allFrames.map((profile, index) => {
            const route = COMBO_ROUTES[profile.key]; const owned = ownedSet.has(profile.key); const reward = profile.unlock?.kind === "reward"; const cost = profile.unlock?.cost; const canBuy = Boolean(cost && progress.credits >= cost); const status = owned ? "OWNED" : reward ? `SECURE LVL ${String(profile.unlock?.level).padStart(2, "0")}` : `${cost} SALVAGE`;
            return <article className={`collection-card ${profile.chassis} ${owned ? "owned" : "locked"} ${comparisonLeft === profile.key || comparisonRight === profile.key ? "comparison-pinned" : ""}`} key={profile.key}>
              <header><span>{profile.collection}</span><b>{profile.callsign}</b></header>
              <div className="collection-pin-row" aria-label={`Pin ${profile.label} to comparison`}><button type="button" className={comparisonLeft === profile.key ? "pinned" : ""} onClick={() => setComparisonLeft(profile.key)}>SLOT A</button><button type="button" className={comparisonRight === profile.key ? "pinned" : ""} onClick={() => setComparisonRight(profile.key)}>SLOT B</button></div>
              <div className="collection-specimen"><FrameSpecimen profile={profile} index={index} /><i className="specimen-grid" /></div>
              <div className="collection-copy"><strong>{profile.label}</strong><p>{profile.description}</p><dl><dt>SPECIAL</dt><dd>Q // {profile.special?.name}</dd><dt>ROUTE</dt><dd>{route.sequence.map((step) => step === "special" ? "Q" : "J").join(" → ")} · {route.finisher}</dd><dt>ROLE</dt><dd>{profile.special?.tactical}</dd></dl></div>
              <footer><span className={`collection-status ${owned ? "owned" : reward ? "reward" : "salvage"}`}>{status}</span>{owned ? <button type="button" onClick={() => { setLoadout(profile.key); setView("hangar"); }}>ASSIGN FRAME</button> : reward ? <button type="button" disabled>LINK REWARD</button> : <button type="button" disabled={!canBuy} onClick={() => buyFrame(profile)}>{canBuy ? `BUY // ${cost}` : `NEED ${cost}`}</button>}</footer>
            </article>;
          })}</div>
        </section>}
