  ...
  return <main className={`mecha-app ${settings.reducedFx ? "fx-reduced" : ""} ${settings.scanlines ? "scanlines-on" : "scanlines-off"} ${settings.contrast ? "contrast-on" : ""}`}>
    ...
    {view === "battle" ? <>
      ...
      <section className="battle-hud" aria-label="Battle interface">
        ...
        <div className="round-dock" aria-live="polite"><div className="round-readout">LVL-{String(hud.missionLevel).padStart(2, "0")} // R0{hud.round}<br />BEST OF 03</div><div className="location-readout"><span>{hud.locationLabel}</span><small>{hud.locationCallout}</small></div>{hud.combo > 0 && <div className={`combo-readout ${hud.overdrive ? "overdrive" : ""}`}><b>{hud.combo}X</b><span>{hud.overdrive ? "OVERDRIVE" : "COMBO"}</span></div>}<div className="status-copy">{hud.message}</div></div>
        <footer className="bottom-deck"><aside className="plate manual"><div className="manual-header"><span className="info-label">{hud.stageLabel}</span><span className="system-line">{hud.playerState.toUpperCase()}</span></div><p>{hud.missionObjective}</p><dl><dt>MOVE</dt><dd>A / D or ← / →</dd><dt>MELEE</dt><dd>J or SPACE</dd><dt>PRISM GUARD</dt><dd>HOLD K</dd><dt>VECTOR BOOST</dt><dd>L or SHIFT</dd><dt>FRAME SPECIAL</dt><dd>Q // {LOADOUTS[hud.selectedLoadout].special?.name ?? "SPECIAL"}</dd><dt>OVERDRIVE</dt><dd>LAND 3 CLEAN HITS</dd></dl></aside><div className="action-cluster"><ActionButton action="strike" className="strike" onAction={onAction}><strong>J</strong>MELEE</ActionButton><ActionButton action="guard" className="guard" onAction={onAction}><strong>K</strong>PRISM GUARD</ActionButton><ActionButton action="boost" className="boost" onAction={onAction}><strong>L</strong>BOOST</ActionButton><ActionButton action="special" className="special" onAction={onAction}><strong>Q</strong>{LOADOUTS[hud.selectedLoadout].special?.name ?? "SPECIAL"}</ActionButton></div><div className="direction-pad"><ActionButton action="left" onAction={onAction}><span>◀</span></ActionButton><ActionButton action="right" onAction={onAction}><span>▶</span></ActionButton></div></footer>
      </section>
      ...
      <div className="console-content">
        ...
        {view === "hangar" && <section className="console-panel hangar-console"><div className="panel-heading"><span>{mission.stageLabel} // LEVEL {String(mission.level).padStart(2, "0")}</span><h2>ASSIGN A CUSTOM FRAME</h2><p>{mission.objective} Hostile: <strong>{mission.opponent.label}</strong>.</p></div><div className="hangar-showcase"><img src={AEGIS_ART} alt="Aegis Rift custom cobalt assault frame" /><div><b>ACTIVE PILOT FRAME</b><strong>{selected.label}</strong><p>{selected.description}</p><span>FRAME SPECIAL // {selected.special?.name} · {selected.special?.tactical}</span></div></div><div className="loadout-grid expanded">{Object.values(LOADOUTS).map((profile) => <button type="button" className={`loadout-card ${profile.chassis} ${loadout === profile.key ? "selected" : ""}`} onClick={() => setLoadout(profile.key as LoadoutKey)} key={profile.key}><span className="loadout-code">{profile.callsign}</span><span className="mecha-portrait" aria-hidden="true"><i className="portrait-head" /><i className="portrait-torso" /><i className="portrait-arm arm-left" /><i className="portrait-arm arm-right" /><i className="portrait-leg leg-left" /><i className="portrait-leg leg-right" /><b className="portrait-core" /></span><strong>{profile.label}</strong><span className="loadout-description">{profile.description}</span><span className="loadout-special">Q // {profile.special?.name}</span><span className="loadout-stats">HP {profile.maxHp} · SPD {profile.speed.toFixed(2)} · DMG {profile.strikeDamage}</span></button>)}</div><div className="hangar-launch-row"><button type="button" className="secondary-console-button" onClick={() => setView("missions")}>BACK TO LEVELS</button><button type="button" className="primary-console-button" onClick={launch}>LAUNCH LEVEL {String(mission.level).padStart(2, "0")}</button></div></section>}
        {view === "roster" && <section className="console-panel"><div className="panel-heading"><span>BLUE FACTION // CUSTOM INVENTORY</span><h2>FRAME ROSTER</h2><p>Each pilot frame has its own silhouette, thruster behavior, defensive response, combat rhythm, and chassis-exclusive special.</p></div><div className="roster-table">{Object.values(LOADOUTS).map((profile) => <article className={`roster-row ${profile.chassis}`} key={profile.key}><div className="mini-mecha"><span className="mini-head" /><span className="mini-body" /><span className="mini-core" /><i /></div><div><b>{profile.label}</b><span>{profile.callsign} · {profile.description}</span><em>Q // {profile.special?.name} · {profile.special?.tactical}</em></div><div className="roster-stat"><small>INTEGRITY</small><strong>{profile.maxHp}</strong></div><div className="roster-stat"><small>SPEED</small><strong>{profile.speed.toFixed(2)}</strong></div><button type="button" onClick={() => { setLoadout(profile.key as LoadoutKey); setView("missions"); }}>FIELD FRAME</button></article>)}</div></section>}
