...
function CustomMecha({ side, action, position, artUrl, chassis, surface }: { side: "player" | "enemy"; action: HudState["playerState"]; position: number; artUrl: string; chassis: string; surface: HudState["surface"] }) {
  const percent = ((position + 6.25) / 12.5) * 100;
  return <div className={`custom-mecha ${side} chassis-${chassis} action-${action} surface-${surface}`} style={{ left: `${Math.max(10, Math.min(90, percent))}%` }} aria-hidden="true">
...
function CombatStage({ hud }: { hud: HudState }) {
  ...
  return <section className={`css-battle-stage theatre-${hud.theatreClass} level-stage-${hud.stage} surface-${hud.surface} atmosphere-${hud.atmosphere}`} aria-hidden="true">
    ...
    <CustomMecha side="player" action={hud.playerState} position={hud.playerX} artUrl={AEGIS_ART} chassis={playerChassis} surface={hud.surface} />
