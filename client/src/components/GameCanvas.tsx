  ...
  return <main className={`mecha-app ${settings.reducedFx ? "fx-reduced" : ""} ${settings.scanlines ? "scanlines-on" : "scanlines-off"} ${settings.contrast ? "contrast-on" : ""}`}>
    ...
    {view === "battle" ? <>
      ...
      <div className="console-content">
        ...
        {view === "roster" && <section className="console-panel collection-console"><div className="collection-heading"><div><span>BLUE FACTION // FRAME COLLECTION</span><h2>ARMORY // {ownedFrames.length}/20 ACTIVE</h2><p>Maintenance-bay records for every recovered chassis. Secure links or spend salvage to bring new frames onto the rail.</p></div><aside><b>{progress.credits.toLocaleString()}</b><span>SALVAGE AVAILABLE</span><small>REWARD FRAMES UNLOCK ON THEIR LINK CLEAR</small></aside></div><div className="collection-grid">{allFrames.map((profile, index) => { const route = COMBO_ROUTES[profile.key]; const owned = ownedSet.has(profile.key); const reward = profile.unlock?.kind === "reward"; const cost = profile.unlock?.cost; const canBuy = Boolean(cost && progress.credits >= cost); const status = owned ? "OWNED" : reward ? `SECURE LVL ${String(profile.unlock?.level).padStart(2, "0")}` : `${cost} SALVAGE`; return <article className={`collection-card ${profile.chassis} ${owned ? "owned" : "locked"}`} key={profile.key}><header><span>{profile.collection}</span><b>{profile.callsign}</b></header><div className="collection-specimen"><FrameSpecimen profile={profile} index={index} /><i className="specimen-grid" /></div><div className="collection-copy"><strong>{profile.label}</strong><p>{profile.description}</p><dl><dt>SPECIAL</dt><dd>Q // {profile.special?.name}</dd><dt>ROUTE</dt><dd>{route.sequence.map((step) => step === "special" ? "Q" : "J").join(" → ")} · {route.finisher}</dd><dt>ROLE</dt><dd>{profile.special?.tactical}</dd></dl></div><footer><span className={`collection-status ${owned ? "owned" : reward ? "reward" : "salvage"}`}>{status}</span>{owned ? <button type="button" onClick={() => { setLoadout(profile.key); setView("hangar"); }}>ASSIGN FRAME</button> : reward ? <button type="button" disabled>LINK REWARD</button> : <button type="button" disabled={!canBuy} onClick={() => buyFrame(profile)}>{canBuy ? `BUY // ${cost}` : `NEED ${cost}`}</button>}</footer></article>; })}</div></section>}
